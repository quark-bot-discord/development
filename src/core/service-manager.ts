...
import { ConfigLoader } from "./service/config-loader.ts";
import { ServiceDependencyResolver } from "./service/dependency-resolver.ts";
import { HealthChecker } from "./service/health-checker.ts";
import { ServiceTypeDetector } from "./service/service-type-detector.ts";
...
export class ServiceManager {
  ...
  async loadK8sConfig(servicePath: string): Promise<KubernetesConfig[]> {
    ...
    try {
      ...
      for (const doc of docs) {
        if (doc && !doc.errors.length) {
          ...
          if (this.isValidKubernetesResource(config)) {
            ...
            Logger.warn(
              `Skipping invalid Kubernetes manifest in ${servicePath}: missing required fields`,
            );
          }
        } else if (doc && doc.errors.length > 0) {
          Logger.warn(
            `YAML parsing errors in ${servicePath}: ${
      ...
      if (configs.length === 0) {
        throw new Error(
          `No valid Kubernetes manifests found in: ${servicePath}`,
        );
      }
      ...
      const errorMessage = `Failed to load K8s config from ${servicePath}: ${
  ...
  async getServiceDependencies(serviceName: string): Promise<string[]> {
    try {
      ...
      Logger.error(
        `Failed to resolve dependencies for ${serviceName}: ${
          err instanceof Error ? err.message : String(err)
        }`,
      );
      return [];
  ...
  async isServiceHealthy(service: string): Promise<boolean> {
    try {
      const healthStatus = await this.healthChecker.checkServiceHealth(
        service,
        "default",
      );
      return healthStatus.status === "healthy";
